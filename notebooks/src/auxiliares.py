...
def analise_shapiro(dataframe, alfa=0.05):
    ...
    for coluna in dataframe.columns:
        ...
        if valor_p_sw > alfa:
            print(f"Coluna {coluna} segue uma distribuição normal (valor p: {valor_p_sw:.3f})")
        else:
            print(
                f"Coluna {coluna} não segue uma distribuição normal (valor p: {valor_p_sw:.3f})"
            )
        print("")
...
def analise_ttest_rel(
    ...
    print("Teste t de Student - Amostras Relacionadas")
    print(f"α = {alfa:.2f}")
